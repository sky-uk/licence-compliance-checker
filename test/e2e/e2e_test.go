...
var _ = Describe("License Compliance Checker", func() {
	...
	It("should fail when project paths do not exist", func() {
		output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "MIT", "testdata/does-not-exist").CombinedOutput()
	...
	It("should find project licence not compliant when is found in the restricted list", func() {
		output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "MIT", "testdata/MIT", "testdata/BSD3").CombinedOutput()
		Expect(err).To(HaveOccurred())
	...
	It("should fail when project does not have license file", func() {
		output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "MIT", "testdata/no-licence").CombinedOutput()
		Expect(err).To(HaveOccurred())
	...
	It("should find project licences not on the restricted list to be compliant", func() {
		output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "BSD", "testdata/MIT", "testdata/BSD3").CombinedOutput()
	...
	It("should find project licence compliant when the restricted licence is overridden", func() {
		output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "MIT", "-o", "testdata/MIT=BSD", "testdata/MIT").CombinedOutput()
		Expect(err).NotTo(HaveOccurred())
	...
	It("should find project licence compliant when the project using the restricted licence is ignored", func() {
		output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "MIT", "-i", "testdata/MIT", "testdata/MIT", "testdata/BSD3").CombinedOutput()
		Expect(err).NotTo(HaveOccurred())
	...
	Context("output", func() {
		It("should not show anything with default options", func() {
			output, err := exec.Command("../../build/bin/licence-compliance-checker", "-r", "BSD", "testdata/MIT").CombinedOutput()
			Expect(err).NotTo(HaveOccurred())
			Expect(string(output)).To(Equal(""))
		})

		It("should show only log messages when only log option is chosen", func() {
			output, err := exec.Command("../../build/bin/licence-compliance-checker", "-L", "info", "-r", "BSD", "testdata/MIT").CombinedOutput()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should not show anything when show-compliance-errors chosen but no compliance checks fail", func() {
			output, err := exec.Command("../../build/bin/licence-compliance-checker", "-E", "-r", "BSD", "testdata/MIT").CombinedOutput()
			Expect(err).NotTo(HaveOccurred())
			Expect(string(output)).To(Equal(""))
		})

		It("should show json output when show-compliance-errors chosen and compliance checks fail", func() {
			output, err := exec.Command("../../build/bin/licence-compliance-checker", "-E", "-r", "MIT", "testdata/MIT").CombinedOutput()
			Expect(err).To(HaveOccurred())
			Expect(string(output)).To(Not(Equal("")))
		})

		It("should show json output when show-compliance-all chosen", func() {
			output, err := exec.Command("../../build/bin/licence-compliance-checker", "-A", "-r", "BSD", "testdata/MIT").CombinedOutput()
			Expect(err).NotTo(HaveOccurred())
			Expect(string(output)).To(Not(Equal("")))
		})
	})
